...
class _GarmentDetailScreenState extends ConsumerState<GarmentDetailScreen> {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildStatusControl(context, ref, garment),
          ],
        ),
      ),
    );
  ...
  Widget _buildStatusControl(
    ...
    if (useSelector) {
      return StatusSelectorWidget(
        ...
        onSelected: (newStatus) => _handleStatusChangeTo(newStatus),
      );
    }
  ...
  Future<void> _handleStatusChangeTo(GarmentStatus to) async {
    ...
    try {
      await ref.read(garmentNotifierProvider.notifier).updateStatus(
        id: garment.id, from: garment.status, to: to,
      );
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              "Estado actualizado a \"${to.displayLabel}\"",
            ),
          ),
        );
      }
